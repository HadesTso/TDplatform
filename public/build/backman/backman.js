    ...
    return {
        ...
        success: function (success, config) {
            ...
            if (success.data.state.code == 10200) {
                ...
                return data;
            }else if (success.data.state.code == 200) {
                var data = success.data.data ? _tools.transKeyName('camel', success.data.data) : {};
                data.__state = _tools.transKeyName('camel', success.data.state);
                return data;
            }
...
backman.directive('bmUploadImg', function (_setting, _httpPost) {
    ...
    return {
        ...
        '<div class="bm-upload-img-input"><input type="file" class="fileInput" name="logo" value="{{bindFile}}"/></div>' +
                ...
                delImg: function () {
                    layer.confirm('<img class="bm-upload-img-del" src="' + $scope.bindUrl + '"/>',
                        {title: '确认删除此图片？'}, function (index) {
                            ...
                            layer.close(index);
                            $scope.bindFile = $scope.bindId = $scope.bindUrl = '';
                            if (!$scope.$$phase && !$scope.$root.$$phase) {
                                $scope.$apply();
                            }
                        }
            ...
            iElm
                .find('input').on('change', function () {
                    ...
                    if (!files[0]) {
                        ...
                        $('.fileInput').attr('type','none')
                    }
                    $scope.bindFile = files[0];
                    $scope.bindUrl = window.URL.createObjectURL(files[0])
                    console.log(files[0])
                    //状态检测
...
backman.directive('bmUploadImgbase64', function (_setting, _httpPost,$timeout) {
    ...
    return {
        ...
        link: function ($scope, iElm, iAttrs) {
                ...
                delImg: function () {
                    layer.confirm('<img class="bm-upload-img-del" src="' + $scope.bindUrl + '"/>',
                        {title: '确认删除此图片？'}, function (index) {
                            ...
                            $(".fileinput").attr('type','file')
                            $scope.state.upAjaxing = false;
                            $scope.bindbase = $scope.bindId = $scope.bindUrl = '';
                            if (!$scope.$$phase && !$scope.$root.$$phase) {
                                $scope.$apply();
                            }
                        }
            ...
            var upload = function (img) {
                ...
                var sendDate = opt.parameters;
                sendDate[opt.fileKeyName] = img.src;
                //$scope.bindbase = sendDate.base64File;
                console.log(sendDate)
                $scope.bindUrl = sendDate.base64File
                $timeout(function () {
                    $('.baseImg').attr('src',sendDate.base64File)
                    $scope.bindbase = sendDate.base64File;
                },100)
                //_httpPost(opt.url, sendDate)
                //    .then(function (data) {
